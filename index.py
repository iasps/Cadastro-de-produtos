...
class IndexUI:
  ...
  def menu_adm():
    op = st.sidebar.selectbox("Menu", ["Manter Produto", "Manter Grupo", "Manter Fabricante", "Editar perfil"])
...
#      st.rerun()
  ...
  def sidebar():
    if "adm_id" not in st.session_state:
      ...
      st.sidebar.write("Bem-vindo(a), " + st.session_state["adm_email"])
      if st.session_state["adm_email"] == "adm": IndexUI.menu_adm()
